...
import {
  acknowledgeWorkflowFieldUpdate,
  loadSingleWorkflowFieldUpdate,
...
export async function POST(req: Request) {
  ...
  if (recordType === 'order') {
    ...
    const canAccessViaShare = canAccessLinkedRecord({
      profile,
    ...
    if (!canAccessViaShare && !canAccessViaCargoRoute) {
  ...
  const fieldUpdate = await loadSingleWorkflowFieldUpdate(serviceSupabase, {
    recordType,
    recordId,
    fieldKey,
  });
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : 'Failed to acknowledge workflow field',
      },
      { status: 500 }
    );
  }
