...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (Object.prototype.hasOwnProperty.call(body, 'segment_order')) {
      const segmentOrder = normalizeSegmentOrder(body.segment_order);
      ...
      payload.segment_order = segmentOrder;
    }

    if (Object.prototype.hasOwnProperty.call(body, 'segment_type')) {
      const segmentType = normalizeSegmentType(body.segment_type);
    ...
    if (Object.keys(payload).length === 0) {
      return NextResponse.json(
        { error: 'No trip leg fields provided for update' },
        { status: 400 }
      );
    }
